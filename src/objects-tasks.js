...
function mergeObjects(objects) {
  return objects.reduce((acc, obj) => {
    Object.entries(obj).forEach((el) => {
      acc[el[0]] = acc[el[0]] ? acc[el[0]] + el[1] : el[1];
    });
    return acc;
  }, {});
}
...
function makeWord(lettersObject) {
  const arr = [];
  Object.values(lettersObject).forEach((el, index) => {
    el.forEach((e) => {
      arr[e] = Object.keys(lettersObject)[index];
    });
  });
  return arr.join('');
}
